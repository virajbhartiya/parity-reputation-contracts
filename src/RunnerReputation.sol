...
contract RunnerReputation {
    ...
    event RunnerRegistered(string indexed runnerId, address indexed walletAddress);
    event ReputationUpdated(string indexed runnerId, int256 oldScore, int256 newScore, string reason);
    event RunnerBanned(string indexed runnerId, address indexed walletAddress, string reason);
    event RunnerUnbanned(string indexed runnerId, address indexed walletAddress);
    ...
    event MonitoringAssigned(string indexed monitorId, string indexed targetId, uint256 duration);
    event MonitoringReportSubmitted(string indexed monitorId, string indexed targetId, uint8 reportType, string evidence);
    event SlashingExecuted(string indexed runnerId, address indexed wallet, uint256 amount, string reason);
    event StakeDeposited(string indexed runnerId, address indexed wallet, uint256 amount);
    event StakeWithdrawn(string indexed runnerId, address indexed wallet, uint256 amount);
    ...
    modifier runnerExists(string memory runnerId) {
        require(bytes(runners[runnerId].runnerId).length > 0, "Runner does not exist");
        _;
    }
    
    modifier notBanned(string memory runnerId) {
        require(runners[runnerId].status != RunnerStatus.Banned, "Runner is banned");
        _;
    }
    
    modifier hasMinimumStake(string memory runnerId) {
        require(runners[runnerId].stakedAmount >= MIN_STAKE_AMOUNT, "Insufficient stake");
        _;
    }
     ...
     */
    function depositStake(string memory runnerId, uint256 amount) external onlyAuthorized {
     ...
     */
    function withdrawStake(string memory runnerId, uint256 amount) external onlyAuthorized runnerExists(runnerId) {
        RunnerProfile storage runner = runners[runnerId];
        require(runner.stakedAmount >= amount, "Insufficient staked amount");
        require(runner.status != RunnerStatus.Banned, "Cannot withdraw while banned");
     ...
     */
    function slashStake(string memory runnerId, string memory reason) external onlyAuthorized runnerExists(runnerId) returns (uint256 slashedAmount) {
        RunnerProfile storage runner = runners[runnerId];
        ...
        // Log slashing event
        reputationEvents[runnerId].push(ReputationEvent({
        ...
        emit SlashingExecuted(runnerId, runner.walletAddress, slashedAmount, reason);
    ...
    function assignRandomMonitoring() external onlyAuthorized {
        ...
        require(activeRunnerCount >= 2, "Need at least 2 active runners for monitoring");
        ...
        for (uint256 i = 0; i < activeRunnerCount && pairsCreated < monitoringPairs; i += 2) {
            ...
            string memory assignmentId = string(abi.encodePacked("monitor-", _uint2str(nextMonitoringId++)));
    ...
    function submitMonitoringReport(
        ...
        MonitoringAssignment storage assignment = monitoringAssignments[assignmentId];
        require(assignment.isActive, "Assignment not active");
        require(!assignment.reportSubmitted, "Report already submitted");
        require(block.timestamp <= assignment.startTime + assignment.duration, "Monitoring period expired");
        ...
        emit MonitoringReportSubmitted(assignment.monitorId, assignment.targetId, uint8(reportType), evidenceHash);
    ...
    function _processMonitoringReport(string memory assignmentId) internal {
        ...
        if (assignment.reportType == MonitoringReportType.GoodBehavior) {
        ...
        } else if (assignment.reportType == MonitoringReportType.PoorPerformance) {
            targetScoreDelta = -10;
            reason = "Poor performance reported by peer monitor";
        } else if (assignment.reportType == MonitoringReportType.SuspiciousActivity) {
            targetScoreDelta = -20;
            reason = "Suspicious activity reported by peer monitor";
        } else if (assignment.reportType == MonitoringReportType.MaliciousBehavior) {
            targetScoreDelta = -50;
            ...
            uint256 slashedAmount = (targetRunner.stakedAmount * SLASHING_PERCENTAGE) / 100;
            ...
            if (slashedAmount > 0) {
                ...
                emit SlashingExecuted(assignment.targetId, targetRunner.walletAddress, slashedAmount, reason);
            }
        } else if (assignment.reportType == MonitoringReportType.Offline) {
            targetScoreDelta = -5;
            reason = "Runner offline during monitoring period";
        } else if (assignment.reportType == MonitoringReportType.ResourceAbuse) {
            targetScoreDelta = -30;
        ...
        if (targetScoreDelta != 0) {
            ...
            reputationEvents[assignment.targetId].push(ReputationEvent({
            ...
            emit ReputationUpdated(assignment.targetId, target.reputationScore - targetScoreDelta, target.reputationScore, reason);
        }
        ...
        reputationEvents[assignment.monitorId].push(ReputationEvent({
        ...
        if (target.reputationScore <= BAN_THRESHOLD && target.status != RunnerStatus.Banned) {
            _banRunner(assignment.targetId, "Reputation dropped below threshold due to peer monitoring");
        }
    ...
    function updateReputation(
        ...
        if (runner.reputationScore <= BAN_THRESHOLD && runner.status != RunnerStatus.Banned) {
            _banRunner(runnerId, "Reputation score below ban threshold");
        }
        else if (runner.reputationScore > QUALITY_THRESHOLD && runner.status == RunnerStatus.Warning) {
            runner.status = RunnerStatus.Active;
            networkStats.activeRunners++;
        }
        else if (runner.reputationScore < QUALITY_THRESHOLD && runner.reputationScore > BAN_THRESHOLD) {
            if (runner.status == RunnerStatus.Active) {
        ...
        emit ReputationUpdated(runnerId, oldScore, runner.reputationScore, reason);
    }
    
    function banRunner(string memory runnerId, string memory reason) external onlyAuthorized runnerExists(runnerId) {
        _banRunner(runnerId, reason);
    }
    
    function registerRunner(string memory runnerId, address walletAddress) external onlyAuthorized {
        require(bytes(runners[runnerId].runnerId).length == 0, "Runner already exists");
        require(bytes(walletToRunnerId[walletAddress]).length == 0, "Wallet already registered");
    ...
    function getMonitoringAssignment(string memory assignmentId) external view returns (MonitoringAssignment memory) {
        return monitoringAssignments[assignmentId];
    }
    
    function getRunnerMonitoringHistory(string memory runnerId) external view returns (string[] memory) {
        return runnerMonitoringHistory[runnerId];
    }
    
    function getActiveMonitoringAssignments() external view returns (string[] memory) {
        return activeMonitoringIds;
    }
    
    function isRunnerEligible(string memory runnerId) external view returns (bool) {
        if (bytes(runners[runnerId].runnerId).length == 0) return false;
        
        RunnerProfile memory runner = runners[runnerId];
        return (runner.status == RunnerStatus.Active || runner.status == RunnerStatus.Warning) &&
               runner.stakedAmount >= MIN_STAKE_AMOUNT;
    }
    
    function getRunnerStatus(string memory runnerId) external view runnerExists(runnerId) returns (
        int256 reputationScore,
        RunnerStatus status,
        uint256 totalTasks,
        uint256 successRate,
    ...
    function getRunnerProfile(string memory runnerId) external view runnerExists(runnerId) returns (RunnerProfile memory) {
        return runners[runnerId];
    }
    
    function getReputationEvents(string memory runnerId) external view runnerExists(runnerId) returns (ReputationEvent[] memory) {
    ...
    function getRunners(uint256 offset, uint256 limit) external view returns (string[] memory) {
    ...
    function getTopRunners(uint256 limit) external view returns (string[] memory topRunners, int256[] memory scores) {
    ...
    function isRunnerBanned(string memory runnerId) external view returns (bool) {
        if (bytes(runners[runnerId].runnerId).length == 0) return false;
    ...
    function _removeFromArray(string[] memory arr, uint256 length, uint256 index) internal pure {
    ...
    function _updateNetworkStats() internal {
        if (networkStats.totalRunners > 0) {
            ...
            networkStats.averageReputation = totalReputation / int256(networkStats.totalRunners);
        }
    }
